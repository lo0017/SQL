...
----Find the name of each company and its average star rating for all companies that have more than 5000 reviews across all locations. How many companies are there with more that 5000 reviews across all locations? ----40
SELECT 
    company, 
    AVG(star_rating) AS avg_rating
FROM data_analyst_jobs
...
----Add the code to order the query in #9 from highest to lowest average star rating. Which company with more than 5000 reviews across all locations in the dataset has the highest star rating? What is that rating? ----Unilever 4.2
SELECT 
    company, 
    AVG(star_rating) AS avg_rating
FROM data_analyst_jobs
...
----Find all the job titles that contain the word ‘Analyst’. How many different job titles are there? ----1669
SELECT title 
FROM data_analyst_jobs 
WHERE title ILIKE '%Analyst%';

----How many different job titles do not contain either the word ‘Analyst’ or the word ‘Analytics’? What word do these positions have in common? ----Tableau
SELECT COUNT(DISTINCT title)
FROM data_analyst_jobs
