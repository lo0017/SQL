...
----Find the name of each company and its average star rating for all companies that have more than 5000 reviews across all locations. How many companies are there with more that 5000 reviews across all locations? ----70
SELECT 
    company, 
    AVG(star_rating) AS avg_rating
FROM data_analyst_jobs
...
----Add the code to order the query in #9 from highest to lowest average star rating. Which company with more than 5000 reviews across all locations in the dataset has the highest star rating? What is that rating? ----Google 4.3
SELECT 
    company, 
    AVG(star_rating) AS avg_rating
FROM data_analyst_jobs
